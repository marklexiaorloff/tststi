...
self.addEventListener('install', event => {
  const urls = [
    'https://lagunalions.com/js/v0.js',
    'https://lagunalions.com/js/amp-install-serviceworker-0.1.js',
...
workbox.routing.registerRoute(/(index|\/news\/)(.*)html|(.*)\/$/, args => {
...
workbox.routing.registerRoute(/(index|\/albums\/)(.*)html|(.*)\/$/, args => {
  return workbox.strategies.networkFirst().handle(args).then(response => {
    if (!response) {
      return caches.match('offline.html');
    }
